...
/**
 * Diálogo para gestionar los productos asignados a una colección (CU08).
 *
 * - Carga los productos asignados con GET /colecciones/{id}/productos.
 * - Carga los productos activos con ProductosService (CU07).
 * - Guarda el conjunto COMPLETO con un único PUT /colecciones/{id}/productos
 *   (nunca un request por checkbox). El PUT es idempotente y una selección
 *   vacía elimina todas las asignaciones.
 *
 ...
 * ("Solo se pueden asignar productos activos") y se muestra el detalle real.
 */
@Component({
  selector: 'app-coleccion-productos-dialog',
  imports: [],
  styleUrl: './coleccion-productos-dialog.css',
  templateUrl: './coleccion-productos-dialog.html',
})
export class ColeccionProductosDialog {
  ...
  /** Ids asignados actualmente en el backend (estado "guardado"). */
  ...
  /** Hay cambios sin guardar respecto al conjunto asignado en el backend. */
  ...
  private cargar(): void {
    ...
    forkJoin({
      asignados: this.coleccionesService.listarProductos(this.coleccion().id),
  ...
  /** Descartar: vuelve al conjunto asignado en el backend. */
  descartar(): void {
    if (this.guardando()) {
      return;
    }
  ...
  guardar(): void {
    ...
    this.coleccionesService
      .reemplazarProductos(this.coleccion().id, ids)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (respuesta) => {
          ...
          this.exito.set(
            respuesta.total === 0
              ? 'Se quitaron todos los productos de la colección.'
              : `Se guardaron ${respuesta.total} producto(s) en la colección.`,
          );
